...
while(zatopione<liczbaOkretow)
	...
	synchronized(lock2){wynik = gra.shoot(id,x,y);
	liczbaStrzalow++;
	...
	if(wynik==true) // otwiera sie nam cale spektrum mozliwosci...
		{
		zatopioneDziala++;
		synchronized(lock2){plansza[x][y]=1;} // w to miejsce juz nie warto strzelac
		if(zatopioneDziala<rozmiarOkretow)
			...
			synchronized(lock1){zatopione++;} // SYNCHONIZACJA!!!!!!!!
			}
		}
	}}
